...
class Listener(object):
    ...
    def __init__(self, _netSection):

        self.size = 1024
        self.socket = None
        self.clientThreads = []
        self.netConf = _netSection
        self.logger = LogHandler.getLogger(__name__)
        for (key, value) in self.netConf.items(self.netConf):
            self.logger.debug("Listener: [" + key + "] => " + str(value))

        # self.netConf = dict(self.netConf.Config.items(self.netConf))
        # self.listenAdress = self.netConf["listenAdress"]

        self.listenAdress = _netSection.get("listenAdress")
        self.listenerPort = _netSection.get("listenerPort")
